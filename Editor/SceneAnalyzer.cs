...
Shaders & Materials:
- Shaders with pass count > 2
- Materials with > MaxShaderKeywords active keywords
- Non-whitelisted shaders flagged as custom/unsupported
...
- Shader duplication (2 assets refering a copy of the same shader) - breaks batching

Lighting:
- Small objects (< MaxSmallObjectVolume) casting shadows from realtime lights — flagged for fragment cost
- Lights with shadows enabled (non-directional)
...
Static Batching:
- Dynamic objects with high vertex count and no motion — advised to mark as static
...
- Small realtime shadow casters → yellow gradient
    ...
    private void AnalyzeScene()
{
    ...
    foreach (var renderer in FindObjectsOfType<Renderer>())
    {
        ...
        foreach (var mat in renderer.sharedMaterials)
        {
            ...
            if (IsGuiShader(shaderName)) continue;
            ...
            if (scanShaders)
            {
                ...
                if (!isWhitelisted)
                    report.Add(($"{Warning} {renderer.name} uses custom or unsupported shader: '{shaderName}'.", renderer.gameObject));
                ...
                if (!scannedShaders.Contains(shader))
                {
                    ...
                    if (!string.IsNullOrEmpty(path))
                    {
                        ...
                        {
                            report.Add(($"{Warning} Shader Graph '{shader.name}' is in use — double check instancing and pass count.", shader));
                        }
                        else if (path.EndsWith(".shader"))
                        {
                            string source = File.ReadAllText(path);

                            bool hasInstancingPragma = source.Contains("#pragma multi_compile_instancing") ||
                                                       source.Contains("#pragma instancing_options");
                            bool hasInstanceMacros = source.Contains("UNITY_VERTEX_INPUT_INSTANCE_ID") &&
                                                     source.Contains("UNITY_SETUP_INSTANCE_ID");

                            if (!hasInstancingPragma || !hasInstanceMacros)
